...
- (void)signIn:(GIDSignIn *)signIn
  ...
  if (_error == nil) {
    if (currentResult_) {
      ...
      NSLog(@"Server Auth Code: %@", currentResult_->serverAuthCode);
      if (!currentResult_->serverAuthCode) {
        NSLog(@"No serverAuthCode received");
      }
    } else {
...
- (void)signIn:(GIDSignIn *)signIn
    didDisconnectWithUser:(GIDGoogleUser *)user
                withError:(NSError *)_error {
  if (_error == nil) {
    NSLog(@"Successfully disconnected user");
  } else {
    NSLog(@"Error disconnecting user: %@", _error);
  }
}
...
void *GoogleSignIn_SignIn() {
  ...
  if (!result) {
    ...
    [[GIDSignIn sharedInstance] signInWithPresentingViewController:UnityGetGLViewController()
        ...
        NSLog(@"Server auth code received: %@", result.serverAuthCode);
        if (!result.serverAuthCode) {
          NSLog(@"No server auth code available!");
        }
    ...
    NSLog(@"Sign-in process already in progress");
  }
  return result;
}
...
void *GoogleSignIn_SignInSilently() {
  SignInResult *result = startSignIn();
  if (!result) {
      NSLog(@"Attempting silent sign-in");
      [[GIDSignIn sharedInstance] restorePreviousSignInWithCompletion:^(GIDGoogleUser *user, NSError *error) {
        if (!error) {
          NSLog(@"Silent sign-in successful for user: %@", user.userID);
        } else {
          NSLog(@"Silent sign-in failed with error: %@", error.localizedDescription);
        }
        [[GoogleSignInHandler sharedInstance] signIn:[GIDSignIn sharedInstance] didSignInForUser:user withError:error];
    }];
    result = currentResult_.get();
  } else {
    NSLog(@"Silent sign-in process already in progress");
  }
  return result;
}
...
void GoogleSignIn_Disconnect() {
  NSLog(@"Disconnecting user");
  GIDSignIn *signIn = [GIDSignIn sharedInstance];
    [signIn disconnectWithCompletion:^(NSError *error) {
      if (error) {
        NSLog(@"Error disconnecting user: %@", error.localizedDescription);
      } else {
        NSLog(@"User successfully disconnected");
      }
